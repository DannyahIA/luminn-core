...
func TestGetAllTasks(t *testing.T) {
	...
	_, err := executeGraphQLRequest(server, createReq)
	require.NoError(t, err)
	...
	resp, err := executeGraphQLRequest(server, getReq)
	require.NoError(t, err)
	require.Empty(t, resp.Errors)
...
func TestCreateAndExecuteTask(t *testing.T) {
	server := setupTestServer()
	defer server.Close()
	...
	createResp, err := executeGraphQLRequest(server, createReq)
	require.NoError(t, err)
	require.Empty(t, createResp.Errors)

	task := createResp.Data["createTask"].(map[string]interface{})
	taskID := task["id"].(string)
	assert.Equal(t, "PENDING", task["status"])

	// Execute task
	executeReq := GraphQLRequest{
		Query: `
	...
	executeResp, err := executeGraphQLRequest(server, executeReq)
	require.NoError(t, err)
	require.Empty(t, executeResp.Errors)

	executedTask := executeResp.Data["executeTask"].(map[string]interface{})
	assert.Equal(t, taskID, executedTask["id"])
	// After execution, task should be COMPLETED (our simple executor completes immediately)
	assert.Equal(t, "COMPLETED", executedTask["status"])
}
...
func TestCreateWorkflow(t *testing.T) {
	...
	createTaskReq := GraphQLRequest{
		...
		Variables: map[string]interface{}{
			"input": map[string]interface{}{
				"name": "Task for Workflow",
			},
		},
	}

	taskResp, err := executeGraphQLRequest(server, createTaskReq)
	require.NoError(t, err)
	require.Empty(t, taskResp.Errors)

	taskID := taskResp.Data["createTask"].(map[string]interface{})["id"].(string)

	// Create workflow
	createWorkflowReq := GraphQLRequest{
		Query: `
	...
	workflowResp, err := executeGraphQLRequest(server, createWorkflowReq)
	require.NoError(t, err)
	require.Empty(t, workflowResp.Errors)

	workflow := workflowResp.Data["createWorkflow"].(map[string]interface{})
	...
	assert.Equal(t, "Task for Workflow", task["name"])
}
